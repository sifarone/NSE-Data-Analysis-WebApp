...
import { cashDataChartAction } from '../../actions/ChartActions';
...
const CashMenu = (props) => {
  ...
  if (props.cashStockSymbolList.length) { 
    ...
    return (
      <ExpansionPanel className={classes.root}>
        <ExpansionPanelSummary
          ...
          id="panel1a-header"
        >
          <Typography variant='body2'>Stock Analysis</Typography>
        </ExpansionPanelSummary>
        ...
        <ExpansionPanelDetails>
          ...
          <InputLabel shrink htmlFor="age-native-simple">Symbol</InputLabel>
            ...
            >
              {props.cashStockSymbolList.map((symbol, i) => <option style={{fontSize: 12}} key={i} value={symbol}>{symbol}</option>)}
            </NativeSelect>
          </FormControl>
        </ExpansionPanelDetails>
